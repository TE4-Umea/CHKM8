...
describe("MYSQL connection and prep", () => {
    ...
    describe("Prepare", () => {
        it("Delete if exists, then creating a test account", async () => {
            ...
            var user = await server.User.create(test_username, "test_password", test_full_name)
            var user2 = await server.User.create(test_username2, "test_password", test_full_name2)
...
describe("Account managing", () => {
    ...
    it("Get user from username and password (safe)", async () => {
        var user = await server.User.get_from_username_and_password(test_username, "test_password")
        assert.notEqual(user, undefined)
    })
})
...
describe("Check in / out testing", () => {
    ...
    it("Test if the user is the owner or joined", async () => {
        var user = await server.User.get_from_username(test_username)
        var project = await server.Project.get(test_project)
...
describe("Projects", () => {
    ...
    it("Add user to project, " + test_username2, async () => {
        ...
        var user2 = await server.User.get_from_username(test_username2)
        var project = await server.Project.get(test_project)
        var added_user = await server.Project.add_user(user2, project.id, user1)
...
describe("Delete user and cleanup", () => {
    it("Delete project", async () => {
        ...
        assert.equal(success.success, true)
        project = await server.Project.get(test_project)
